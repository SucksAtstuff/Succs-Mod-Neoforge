...
public class HammerItem extends DiggerItem {
    ...
    public void appendHoverText(ItemStack stack,
        ...
        int widenLvl = EnchantmentHelper.getItemEnchantmentLevel(widening, stack);
        int depthLvl = EnchantmentHelper.getItemEnchantmentLevel(depth, stack);
     ...
     */
    public static List<BlockPos> getVolumeTargets(ServerPlayer player, BlockPos origin,
                                                  int widthRange, int baseDepthRange, int totalDepthRange) {
        List<BlockPos> out = new ArrayList<>();

        // Resolve face (ray first; if MISS, infer from look vector)
        BlockHitResult hit = player.level().clip(new ClipContext(
                player.getEyePosition(1f),
                player.getEyePosition(1f).add(player.getViewVector(1f).scale(6f)),
                ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, player));
        ...
        if (hit.getType() != HitResult.Type.MISS) {
            ...
            var v = player.getViewVector(1f);
            if (Math.abs(v.y) >= Math.abs(v.x) && Math.abs(v.y) >= Math.abs(v.z)) {
                face = v.y >= 0 ? Direction.UP : Direction.DOWN;
            } else if (Math.abs(v.x) >= Math.abs(v.z)) {
                face = v.x >= 0 ? Direction.EAST : Direction.WEST;
            } else {
                face = v.z >= 0 ? Direction.SOUTH : Direction.NORTH;
            }
        }

        // Step INTO the block (forward-only)
        Direction into = face.getOpposite();
        Direction.Axis axis = into.getAxis();
        int dir = into.getAxisDirection().getStep(); // +1 toward interior

        // For each column in the widened plane, walk forward and STOP when air/replaceable is found
        for (int a = -widthRange; a <= widthRange; a++) {
            for (int b = -widthRange; b <= widthRange; b++) {
                for (int c = 0; c <= 2 * totalDepthRange; c++) {
                    int dx = 0, dy = 0, dz = 0;
                    switch (axis) {
                        case X -> { dy = a; dz = b; dx = dir * c; }
                        case Y -> { dx = a; dz = b; dy = dir * c; }
                        case Z -> { dx = a; dy = b; dz = dir * c; }
                    }
                    BlockPos pos = origin.offset(dx, dy, dz);

                    // STOP extending this column if we hit air/replaceable
                    var state = player.level().getBlockState(pos);
                    if (state.isAir()) {
                        break; // break depth loop for this (a,b) column
                    }
