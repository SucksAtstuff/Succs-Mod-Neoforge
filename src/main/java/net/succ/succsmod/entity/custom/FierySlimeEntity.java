...
import net.succ.succsmod.entity.goals.FierySlimeAttackGoal;
import net.succ.succsmod.entity.goals.ToxicSlimeAttackGoal;
import org.jetbrains.annotations.Nullable;

import java.util.List;

public class FierySlimeEntity extends Slime {

    public FierySlimeEntity(EntityType<? extends Slime> type, Level level) {
        super(type, level);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new FierySlimeAttackGoal(this));
    ...
    public void setSize(int size, boolean resetHealth)
    {
        ...
        this.getAttribute(Attributes.MAX_HEALTH).setBaseValue(size * 5.0D); // More HP per size
    ...
    public boolean doHurtTarget(Entity target) {
        if (super.doHurtTarget(target)) {
            if (target instanceof LivingEntity living) {
                // Apply poison effect
                living.addEffect(new MobEffectInstance(ModEffects.TRUE_FIRE_EFFECT, 60, 1));
            }
            return true;
        }
        return false;
    ...
    private void createTrueFireAura() {
        ...
        for (LivingEntity entity : nearby) {
            entity.addEffect(new MobEffectInstance(ModEffects.TRUE_FIRE_EFFECT, 20, 0));
        }
    }

    public static boolean checkFierySlimeSpawnRules(EntityType<FierySlimeEntity> entityType,
                                                    LevelAccessor level,
    ...
    public void die(DamageSource source) {
        ...
        if (!this.level().isClientSide && size > 1 && this.isDeadOrDying()) {
            ...
            for (int i = 0; i < splitCount; ++i) {
                ...
                FierySlimeEntity mini = new FierySlimeEntity(this.getType(), this.level());
